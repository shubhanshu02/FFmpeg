...
static DNNReturnType fill_model_input_tf(TFModel *tf_model, TFRequestItem *request) {
    ...
    TFContext *ctx = &tf_model->ctx;
    uint64_t offset = 0;
    int size = ctx->options.batch_size;


    if (size > ff_queue_size(tf_model->inference_queue)) {
        size = ff_queue_size(tf_model->inference_queue);
    }
    ...
    infer_request->input_tensor = allocate_input_tensor(&input, size);
...
static void infer_completion_callback(void *args) {
    ...
    for (int j = 0; j < request->inference_count; ++j) {
        task = request->inferences[j]->task;
        task->inference_done++;
